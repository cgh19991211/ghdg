			...
			<div class="column is-one-quarter">
				<el-card shadow="never">
					<div slot="header" class="has-text-centered">
						...
						<p class="mt-3">{{ topicUser.nickname || topicUser.bloggerName }}</p>
					</div>
					<div>
						<p class="content">入驻：{{ dayjs(topicUser.registrationDate).format("YYYY/MM/DD HH:MM:ss") }}</p>
						<p class="content">简介：{{ topicUser.signature }}</p>
					</div>
				</el-card>
			</div>
							...
							<div class="media-content">
								...
								<nav class="level has-text-grey is-size-7">
									<div class="level-left">
										<span class="mr-1">
											发布于:{{ dayjs(item.createdDate).format("YYYY/MM/DD HH:mm:ss") }}
										</span>
									</div>
								</nav>
							</div>
							<div v-if="BloggerAccessToken" class="media-right">
								<div v-if="topicUser.bloggerId === user.bloggerId" class="level">
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters(['BloggerAccessToken', 'user'])
		},
		created() {
			this.fetchUserById()
		},
